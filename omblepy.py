...
class bluetoothTxRxHandler:
    ...
    def __init__(self, ble_client, pairing=False):
        ...
        self.rxChannelUuidToIndex = {
            uuid: idx for idx, uuid in enumerate(self.deviceRxChannelUUIDs)
        }
        ...
        if(self.currentRxNotifyStateFlag != True):
            ...
            for rxChannelUUID in self.deviceRxChannelUUIDs:
                try:
                    ...
                    logger.exception(f"Failed notify callback on {rxChannelUUID}")
            self.currentRxNotifyStateFlag = notify_success_count > 0
            if notify_success_count == 0:
                raise ValueError("failed to enable any RX notification channel")
    ...
    def _resolveRxChannelId(self, BleakGATTChar):
        if hasattr(BleakGATTChar, "uuid") and BleakGATTChar.uuid in self.rxChannelUuidToIndex:
            return self.rxChannelUuidToIndex[BleakGATTChar.uuid]

        if type(BleakGATTChar) is int and BleakGATTChar in self.deviceDataRxChannelIntHandles:
            return self.deviceDataRxChannelIntHandles.index(BleakGATTChar)

        if hasattr(BleakGATTChar, "handle") and BleakGATTChar.handle in self.deviceDataRxChannelIntHandles:
            return self.deviceDataRxChannelIntHandles.index(BleakGATTChar.handle)

        return None

    def _callbackForRxChannels(self, BleakGATTChar, rxBytes):
        rxChannelId = self._resolveRxChannelId(BleakGATTChar)
        if rxChannelId is None:
            logger.warning(
                f"Ignoring rx from unknown characteristic: {getattr(BleakGATTChar, 'uuid', BleakGATTChar)} "
                f"handle={getattr(BleakGATTChar, 'handle', 'n/a')} data={convertByteArrayToHexString(rxBytes)}"
            )
            return
    ...
    async def startTransmission(self):
        ...
        if(self.rxPacketType != bytearray.fromhex("8000")):
            raise ValueError(
                "invalid response to data readout start "
                f"packetType={convertByteArrayToHexString(self.rxPacketType or b'')} "
                f"data={convertByteArrayToHexString(self.rxDataBytes or b'')}"
            )
    ...
    async def endTransmission(self):
        ...
        logger.debug(f"endTransmission rxPacketType={convertByteArrayToHexString(self.rxPacketType or b'')} rxDataBytes={convertByteArrayToHexString(self.rxDataBytes or b'')}")
        if(self.rxPacketType != bytearray.fromhex("8f00")):
            raise ValueError("invalid response to data readout end")
            return
        # rxDataBytes[0] == 0 means success; some devices return non-zero status that is not an error
        if self.rxDataBytes and len(self.rxDataBytes) > 0 and self.rxDataBytes[0] != 0:
            logger.warning(f"endTransmission: device status byte = {self.rxDataBytes[0]} (non-zero, treating as OK)")
